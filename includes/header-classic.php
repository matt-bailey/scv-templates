<header id="header">

    <div class="header-top-wrapper">
        <div class="header-top">

            <div class="header-logo">
                <a class="logo" href="#">
                    <img src="images/scv-logo-classic.png" />
                </a>
            </div><!-- .header-logo -->

            <div class="header-search">
                <form>
                    <input type="text" id="search" class="input-xlarge" placeholder="Search"><i class="icon-search"></i>
                </form>
            </div><!-- .header-search -->

            <div class="header-buttons">
                <button class="btn btn-primary" href="#"><i class="icon-pencil"></i> Blog</button> <button class="btn btn-primary" href="#"><i class="icon-envelope-alt"></i> Email Us</button>
            </div><!-- .header-buttons -->
            
        </div>
    </div><!-- .header-top-wrapper -->

    <div class="header-bottom-wrapper">
        <div class="header-bottom">
            <div class="header-bottom-inner">

                <div class="phone-numbers">
                    <span class="phone-number landline"><i class="entypo-phone entypo-large entypo-orange"></i>01793 509 311</span>
                    <span class="phone-number mobile"><i class="entypo-mobile entypo-large entypo-orange"></i>07860 776 578</span>
                </div><!-- .phone-numbers -->

                <div class="navbar">
                    <nav class="navbar-inner">
                        <ul class="nav">
                            <li class="first active"><a href="#"><i class="icon-home"></i></a></li>
                            <li><a href="#">Fleet &amp; Prices</a></li>
                            <li><a href="#">Location</a></li>
                            <li><a href="#">About Us</a></li>
                            <li><a href="#">Contact Us</a></li>
                            <li><a href="#">Special Offers</a></li>
                            <li class="last"><a href="#">Contract Hire</a></li>
                        </ul>
                    </nav>
                </div><!-- .navbar -->

            </div>
        </div>
    </div><!-- .header-bottom-wrapper -->

</header><!-- .header -->
